...
export type MacroEffectsV1 = {
  ...
  property_purchase_discount_pct?: number; // applies only to direct property purchases from the bank
...
export const MACRO_DECK_PH_V1: MacroCardV1[] = [
  {
    id: "steady-growth",
    name: "Steady Growth",
    rarity: "common",
  ...
  },
  {
    ...
    rulesText:
      "Randomly select 2 color sets. Pay ₱400,000 per house on properties in those sets.",
    effects: {
      regional_disaster: {
        colorSets: 2,
        costPerHouse: 400000,
  ...
  },
  {
    ...
    rulesText:
      "All players skip their next roll. Loans and mortgages still tick. Each player receives ₱800,000 stimulus.",
    effects: {
      pandemic: {
        skipNextRoll: true,
        stimulusCash: 800000,
  ...
  },
  {
    ...
    rulesText:
      "Each player pays ₱1,600,000 immediately. If cash is insufficient, liquidate houses until covered.",
    effects: {
      sovereign_default: {
        cashDelta: -1600000,
...
export const drawMacroCardPhV1 = (lastCardId?: string | null) => {
  if (MACRO_DECK_PH_V1.length === 0) {
    throw new Error("Macro deck ph v1 is empty.");
  }
  const filtered =
    lastCardId == null
      ? MACRO_DECK_PH_V1
      : MACRO_DECK_PH_V1.filter((card) => card.id !== lastCardId);
  const candidates = filtered.length > 0 ? filtered : MACRO_DECK_PH_V1;
  return pickWeighted(candidates);
};

export function getMacroCardByIdPhV1(id: string): MacroCardV1 | undefined {
  return MACRO_DECK_PH_V1.find((card) => card.id === id);
}
