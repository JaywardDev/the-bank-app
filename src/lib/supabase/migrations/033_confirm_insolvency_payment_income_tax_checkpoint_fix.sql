  ...
  if pending_reason = 'PAY_RENT' then
    ...
    if loan_status = 'paid' then
      ...
      values (
        ...
        jsonb_build_object(
          ...
          'loan_id', loan_row.id,
          'tile_index', pending_tile_index,
          'tile_name', coalesce(ownership_row.tile_name, pending_label)
        ),
        now(),
        actor_user_id
      );
    ...
    values (
      ...
      jsonb_build_object(
        ...
        'turns_remaining', case when found then coalesce(loan_row.turns_remaining, 0) else null end,
        'status', case when found then coalesce(loan_row.status, 'active') else null end
      ),
      now(),
      actor_user_id
    );
