...
begin
  ...
  foreach tile_index in array coalesce(trade_row.offer_tile_indices, '{}'::integer[]) loop
    select *
      into ownership_row
      from public.property_ownership
      where property_ownership.game_id = p_game_id
        and property_ownership.tile_index = tile_index
      for update;

    if not found or ownership_row.owner_player_id is null then
      stale_reason := format('Tile %s is no longer owned.', tile_index);
      exit;
    end if;

    if ownership_row.owner_player_id <> trade_row.proposer_player_id then
      stale_reason := format('Proposer no longer owns tile %s.', tile_index);
      exit;
    end if;
  end loop;

  if stale_reason is null then
    foreach tile_index in array coalesce(trade_row.request_tile_indices, '{}'::integer[]) loop
      select *
        into ownership_row
        from public.property_ownership
        where property_ownership.game_id = p_game_id
      ...
      if ownership_row.owner_player_id <> trade_row.counterparty_player_id then
        stale_reason := format('Counterparty no longer owns tile %s.', tile_index);
        exit;
      end if;
    end loop;
  end if;
  ...
  if stale_reason is null then
    for snapshot_tile in select value from jsonb_array_elements(snapshot_tiles) loop
      ...
      select *
        ...
        from public.property_ownership
        where property_ownership.game_id = p_game_id
          and property_ownership.tile_index = snapshot_tile_index
        for update;
