alter table public.purchase_mortgages
  add column if not exists turns_remaining integer not null default 0;

alter table public.purchase_mortgages
  add column if not exists payment_per_turn integer not null default 0;

update public.purchase_mortgages
set
  turns_remaining = greatest(coalesce(term_turns, 0) - coalesce(turns_elapsed, 0), 0),
  payment_per_turn = coalesce(payment_per_turn, 0)
where true;

alter table public.game_state
  alter column rules set default '{"freeParkingJackpotEnabled": false, "loanCollateralEnabled": true, "mortgageRatePerTurn": 0.015, "mortgageTermTurns": 30, "mortgageLtv": 0.5, "collateralLtv": 0.5, "collateralRatePerTurn": 0.008, "collateralTermTurns": 10, "loanRatePerTurn": 0.008, "loanTermTurns": 10, "auctionEnabled": true, "auctionMinIncrement": 10, "auctionTurnSeconds": 60, "auctionAllowInitiatorToBid": true, "macroEnabled": true}'::jsonb;

drop function if exists public.take_collateral_loan(
  uuid,
  uuid,
  integer,
  integer,
