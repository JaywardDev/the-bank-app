create or replace function public.take_collateral_loan(
  ...
  tile_id text,
  actor_user_id uuid,
  collateral_ltv_input numeric default null,
  rate_per_turn_input numeric default null,
  term_turns_input integer default null,
  collateral_principal_input integer default null
)
returns table (
  game_state jsonb,
...
declare
  ...
  collateral_event_version integer;
  cash_event_version integer;
...
begin
  ...
  if exists (
    select 1
      ...
      where l.game_id = take_collateral_loan.game_id
        and l.collateral_tile_index = take_collateral_loan.tile_index
        and l.status = 'active'
  ) then
    raise exception using message = 'ALREADY_COLLATERALIZED', errcode = 'P0001';
  end if;

  collateral_ltv := coalesce(
    collateral_ltv_input,
    (state_row.rules ->> 'collateralLtv')::numeric,
    0.5
  );
  rate_per_turn := coalesce(
    rate_per_turn_input,
    (state_row.rules ->> 'collateralRatePerTurn')::numeric,
    (state_row.rules ->> 'loanRatePerTurn')::numeric,
    0.008
  );
  term_turns := coalesce(
    term_turns_input,
    (state_row.rules ->> 'collateralTermTurns')::integer,
    (state_row.rules ->> 'loanTermTurns')::integer,
    10
  );

  principal := coalesce(collateral_principal_input, round(tile_price * collateral_ltv)::integer);
  if principal <= 0 then
    raise exception using message = 'INVALID_PRICE', errcode = 'P0001';
  end if;
  ...
  collateral_event_version := state_row.version + 1;
  cash_event_version := state_row.version + 2;
  final_version := cash_event_version;
  ...
  (
    ...
    jsonb_build_object(
      'player_id', player_id,
      'amount', principal,
      'reason', 'COLLATERAL_LOAN_TAKEN',
      'tile_index', tile_index,
      'loan_id', loan_row.id,
      'source_event_type', 'COLLATERAL_LOAN_TAKEN'
    ),
    now(),
    actor_user_id
  );
  ...
  when unique_violation then
    ...
    begin
      ...
      raise exception using
        message = concat('UNIQUE_VIOLATION:', coalesce(v_constraint, 'unknown')),
        errcode = 'P0001';
    end;
end;
$$;
...
begin
  ...
  payoff_event_version := state_row.version + 1;
  cash_event_version := state_row.version + 2;
  final_version := cash_event_version;
  ...
  (
    ...
    jsonb_build_object(
      'player_id', player_id,
      'amount', payoff_amount,
      'reason', 'COLLATERAL_LOAN_PAYOFF',
      'tile_index', loan_row.collateral_tile_index,
      'loan_id', loan_row.id,
      'source_event_type', 'LOAN_PAID_OFF'
    ),
    now(),
    actor_user_id
  );
